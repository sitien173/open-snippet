...
fn auto_orchestrator() -> Orchestrator<MockSink, MockClipboardBackend> {
    let injector = Injector::new_with_sink(MockSink::default());
    let mut orchestrator = Orchestrator::new_with_state(
        vec![snippet(";sig", "hello")],
        injector,
...
async fn manual_mode_arms_on_full_trigger_without_immediate_injection() {
    let _guard = test_guard();
    let mut orchestrator = manual_orchestrator();

    for ch in ";sig".chars() {
        let injected = orchestrator.handle_event(HookEvent::Char(ch)).unwrap();
        assert!(!injected);
    }
...
#[tokio::test(flavor = "current_thread")]
async fn tab_confirm_expands_after_manual_arm() {
    let _guard = test_guard();
    let mut orchestrator = manual_orchestrator();
    for ch in ";sig".chars() {
...
#[tokio::test(flavor = "current_thread")]
async fn enter_confirm_expands_after_manual_arm() {
    let _guard = test_guard();
    let mut orchestrator = manual_orchestrator();
    for ch in ";sig".chars() {
        let _ = orchestrator.handle_event(HookEvent::Char(ch)).unwrap();
    ...
    assert!(injected);
    assert_eq!(orchestrator.injector().sink().actions.len(), 9);
}

#[tokio::test(flavor = "current_thread")]
async fn auto_mode_preserves_immediate_expansion() {
    let _guard = test_guard();
    let mut orchestrator = auto_orchestrator();

    let mut injected = false;
    for ch in ";sig".chars() {
        injected = orchestrator.handle_event(HookEvent::Char(ch)).unwrap();
    }
